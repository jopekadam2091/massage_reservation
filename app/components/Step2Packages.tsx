  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-4 items-stretch pt-3">
        {packagesData[selectedType].map((pkg: PackageItem) => {
          ...
          return (
            ...
            >
              {isMiddle && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 whitespace-nowrap bg-violet-600 text-white text-[10px] font-extrabold px-3.5 py-1.5 rounded-full shadow-md uppercase tracking-wider z-20 flex items-center gap-1">
              ...
              <div
                className={`flex flex-col h-full rounded-3xl border transition-all overflow-hidden relative ${
                  isMiddle
                    ? 'bg-gradient-to-b from-violet-50/60 via-white to-white dark:from-violet-950/40 dark:via-zinc-800 dark:to-zinc-800 border-2 border-violet-500 shadow-xl shadow-violet-500/10'
                    : 'bg-white/80 dark:bg-zinc-800/80 border-slate-200 dark:border-zinc-700'
                }`}
              >
                  ...
                  <span
                    className={`px-3 py-1 text-[11px] font-bold tracking-wider uppercase rounded-full mb-4 ${
                      isMiddle 
                        ...
                        : 'bg-slate-100 dark:bg-zinc-700/80 text-slate-600 dark:text-zinc-300'
                    }`}
                  >
                    {pkg.badge}
                  </span>
                  <div className="flex items-baseline mb-1 text-slate-800 dark:text-zinc-100">
                    <span className={`font-black tracking-tight ${isMiddle ? 'text-5xl text-violet-600 dark:text-violet-400' : 'text-3xl'}`}>
                      {priceStr.split(' ')[0]}
                    </span>
                    <span className="text-lg font-bold ml-1 text-slate-500 dark:text-zinc-400">eur</span>
                    <span className="text-xs font-semibold text-slate-400 dark:text-zinc-500 ml-2">/ {pkg.duration} {t.minutes}</span>
                  </div>
                  <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1 max-w-[200px]">{pkg.desc}</p>
                </div>

                {/* 3. Tlačidlo s prémiovým violet-purple gradientom */}
                <div className="p-6 pt-4">
                  <button
                    ...
                    className={`w-full rounded-xl font-bold text-xs uppercase tracking-wider transition-all active:scale-95 ${
                      isMiddle
                        ? 'py-4 bg-gradient-to-r from-violet-600 via-purple-600 to-amber-600 text-white shadow-md hover:shadow-violet-500/20 hover:brightness-110'
                        : 'py-3 bg-slate-100 dark:bg-zinc-700/80 border border-slate-200 dark:border-zinc-600 text-slate-700 dark:text-zinc-200 hover:bg-violet-600 hover:text-white'
                    }`}
                  >
                    {t.selectBtn}
                  </button>
                ...
                <div className="border-t border-slate-100 dark:border-zinc-700 my-2 mx-6"></div>
                      ...
                      <li key={idx} className="flex items-start space-x-2">
                        <span className="w-4 flex-shrink-0 text-center font-bold mt-0.5">
                          {feat.icon === 'chili' ? (
                            ...
                            <Check size={14} className="text-violet-600 dark:text-violet-400 inline" />
                          )}
                        </span>
                        <span className={isMiddle ? 'text-slate-800 dark:text-zinc-200' : 'text-slate-600 dark:text-zinc-400'}>{feat.text}</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <button
        type="button"
        onClick={onBack}
        className="mx-auto flex items-center justify-center px-6 py-3 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-bold text-xs tracking-wider uppercase hover:bg-slate-100 dark:hover:bg-zinc-700 transition shadow-sm active:scale-95"
      >
        {t.backToLevel}
      </button>
    </div>
