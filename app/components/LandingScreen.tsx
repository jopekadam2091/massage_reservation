...
import { Sparkles, ShieldCheck, Star, QrCode, ArrowRight } from 'lucide-react';
...
export default function LandingScreen({ onEnter, onViewServices }: Props) {
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center p-4 sm:p-8 pt-20 text-center overflow-hidden font-sans">
      {/* Main Evervault Surface Card */}
      <div className="relative z-10 w-full max-w-xl p-8 sm:p-12 rounded-2xl bg-[#0B0D22]/95 backdrop-blur-2xl border border-[#2B2F49] shadow-2xl space-y-8 animate-fadeIn text-[#DDE0F2]">
        
        {/* Interactive SVG Pen-Drawing Logo Container */}
        <div className="relative mx-auto w-32 h-32 sm:w-36 sm:h-36 flex items-center justify-center group cursor-pointer">
          {/* Ambient Glow Aura on Hover */}
          <div className="absolute inset-0 rounded-full bg-[#6633EE]/0 blur-2xl opacity-0 group-hover:opacity-100 group-hover:bg-[#6633EE]/30 transition-all duration-500" />
          
          <div className="relative w-full h-full p-4 flex items-center justify-center rounded-full border border-[#2B2F49] group-hover:border-[#6633EE]/60 transition-all duration-500 bg-[#010314]/60">
            {/* Vector SVG Logo with Pen Stroke Animation */}
            <svg
              id="Hand_Logo_SVG"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 500 414.01"
              className="w-full h-full text-[#6633EE] drop-shadow-[0_0_16px_rgba(102,51,238,0.6)]"
            >
              <g id="Hand_Logo">
                <path
        ...
        <div className="space-y-3">
          <span className="badge-purple inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-medium">
            <Sparkles size={13} className="text-[#A78BFA]" />
            Privátny Masážny Salón
          </span>
          <h1 className="text-3xl sm:text-4xl font-semibold text-[#FFFFFF] tracking-tight leading-tight">
            ZenFlow Massage Sanctuary
          </h1>
          <p className="text-sm text-[#C7CAE0] max-w-md mx-auto font-normal leading-relaxed">
            Obnovte rovnováhu tela a mysle prostredníctvom precíznej regenerácie v exkluzívnom komornom prostredí.
          </p>
        </div>

        {/* Trust Badges Grid */}
        <div className="grid grid-cols-3 gap-3 pt-2">
          <div className="p-3.5 rounded-xl bg-[#010314]/80 border border-[#2B2F49] flex flex-col items-center justify-center gap-1">
            <Star size={16} className="text-[#A78BFA]" />
            <span className="text-xs font-medium text-[#FFFFFF]">4.9 ★ Rating</span>
            <span className="text-[10px] text-[#C7CAE0]/60">500+ Hodín</span>
          </div>

          <div className="p-3.5 rounded-xl bg-[#010314]/80 border border-[#2B2F49] flex flex-col items-center justify-center gap-1">
            <ShieldCheck size={16} className="text-[#A78BFA]" />
            <span className="text-xs font-medium text-[#FFFFFF]">100% Privát</span>
            <span className="text-[10px] text-[#C7CAE0]/60">Maximálny Relax</span>
          </div>

          <div className="p-3.5 rounded-xl bg-[#010314]/80 border border-[#2B2F49] flex flex-col items-center justify-center gap-1">
            <QrCode size={16} className="text-[#A78BFA]" />
            <span className="text-xs font-medium text-[#FFFFFF]">QR Lístok</span>
            <span className="text-[10px] text-[#C7CAE0]/60">Okamžitý Vstup</span>
          </div>
        </div>
        ...
        <div className="pt-3 flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            type="button"
            onClick={onEnter}
            className="w-full sm:w-auto min-w-[200px] btn-primary text-sm font-medium active:scale-95 flex items-center justify-center gap-2"
          >
            <span>Rezervovať Masáž</span>
            <ArrowRight size={16} />
          </button>
          ...
          {onViewServices && (
            <button
              ...
              className="w-full sm:w-auto btn-secondary text-sm font-medium active:scale-95"
            >
              <span>Zobraziť Cenník</span>
            </button>
          )}
        </div>
      </div>
