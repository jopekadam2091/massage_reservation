          ...
          <main className="max-w-4xl mx-auto p-6 mt-10">
            {mode === 'photo' ? (
              ...
              <div className="space-y-8">
                ...
                <form onSubmit={(e) => { e.preventDefault(); alert('Sent!'); }} className="bg-neutral-900 p-6 rounded-xl border border-neutral-800 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select className="w-full p-3 bg-neutral-800 border border-neutral-700 rounded text-white">
                        <option>Portrét</option><option>Budoár</option><option>Umelecká</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-400 uppercase mb-2">{t.environment}</label>
                      <select className="w-full p-3 bg-neutral-800 border border-neutral-700 rounded text-white">
                        <option>{t.interior}</option><option>{t.exterior}</option>
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase mb-2">{t.descLabel}</label>
                    <textarea rows={4} placeholder={t.descPlaceholder} className="w-full p-3 bg-neutral-800 border border-neutral-700 rounded text-white" required />
                  </div>
                    ...
                    <div className="space-y-6">
                      ...
                      {selectedSlot && (
                        <form onSubmit={(e) => { e.preventDefault(); alert('Reserved!'); }} className="space-y-4 pt-4 border-t border-gray-100">
                          ...
                          <input type="text" placeholder={t.name} required className="w-full p-3 border rounded text-sm bg-gray-50" />
